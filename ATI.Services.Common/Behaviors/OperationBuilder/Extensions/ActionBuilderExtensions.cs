...
namespace ATI.Services.Common.Behaviors.OperationBuilder.Extensions
{
    ...
    public static class ActionBuilderExtensions
    {
        ...
        public static ActionBuilder WithActionResultRewrite(this ActionBuilder builder, Dictionary<ActionStatus, IActionResult> resultRewrites)
        {
        ...
        public static T WithCustomStatus<T>(this T action, Func<ActionStatus, HttpStatusCode?> customStatusFunc) where T : BaseActionBuilder
        {
            action.GetCustomStatus = customStatusFunc;
            return action;
        }
        
        public static T WithCustomErrorCode<T>(this T action, Func<ActionStatus, string> customErrorCodeFunc) where T : BaseActionBuilder
        {
            action.GetCustomErrorCode = customErrorCodeFunc;
            return action;
        }
        
        public static T WithCustomMessage<T>(this T action, Func<ActionStatus, string> customMessageFunc) where T : BaseActionBuilder
        {
            action.GetCustomMessage = customMessageFunc;
            return action;
        }

        public static T WithCustomMessage<T>(this T action, IDictionary<ActionStatus, string> customMessageMap) where T : BaseActionBuilder
        {
            action.GetCustomMessage = status => customMessageMap.TryGetValue(status, out var message) ? message : null;
            return action;
        }
        
        public static TFunc WithNotFoundCondition<TFunc, T>(this TFunc function, Func<T, bool> notFoundCondition) where TFunc : BaseFunctionBuilder<T>
        {
